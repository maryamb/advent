...
fn get_seeds(line: &str) -> HashMap<u64, u64> {
  let all_nums = line
    .split(':')
    .nth(1)
    .unwrap_or("")
...
impl SeedsAndMaps {
  ...
  fn seeds_to_min_location(&self, kv: (u64, u64)) -> u64 {
    (kv.0..kv.0 + kv.1)
      .into_iter()
  ...
  fn get_min_seeds_location(&self) -> u64 {
    self.seeds.clone()
      .into_iter()
      .map(|kv| self.seeds_to_min_location(kv))
      .min()
      .expect("It is not empty.")
  }
}
...
fn main() {
  let sam = make_sam("data/d5_p1.txt").expect("File exists.");
